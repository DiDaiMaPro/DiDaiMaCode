...
namespace ExpressCode.Common
{
    public class UploadFilesHelper
    {
        ...
        public UploadFilesHelper(IHostingEnvironment hosting, string url=null)
        {
            _url = url;
            _ev = hosting;
            //判断字典是否有数据如果没有数据进行添加
        ...
        private string Save(IFormFile file, string suffix)
        {
            ...
            //保存到本地
            using (var fs = System.IO.File.Create(_ev.WebRootPath + $"\\img\\" + myFileName))
            {
                file.CopyTo(fs);  //把文件拷贝到fs这个绝对路径
                fs.Flush();          //刷新磁盘
            }
            //返回文件路径
            return $"/img/" + myFileName;
        }
    }
}
